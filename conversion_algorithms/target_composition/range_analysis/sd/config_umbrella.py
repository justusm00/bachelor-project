...
# @Last Modified time: 2023-06-30 16:53:50
import h5py
import numpy as np
import sys

#routine to adjust umbrella field, area51 and minimum annealing temperature
##usage: python3 config_umbrella.py <coordfile> <delta_phi> <SA> <total_density> <lamella width>

coordfile=sys.argv[1] ##file to be modified
delta_phi=float(sys.argv[2]) ##delta phi
sa =int(sys.argv[3]) 
total_density=float(sys.argv[4])
lamella_width=int(sys.argv[5])
print(f"lamella_width: {lamella_width}")

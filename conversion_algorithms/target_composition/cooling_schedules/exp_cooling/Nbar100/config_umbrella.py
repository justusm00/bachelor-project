...
# @Last Modified time: 2023-05-15 14:44:53
import h5py
import numpy as np
import sys

#routine to adjust umbrella field, area51 and minimum annealing temperature
##usage: python3 config_umbrella.py <coordfile> <delta_phi> <alpha>
...
dset_Tmin = f.create_dataset("parameter/Tmax", data=np.array([Tmax]))
e = "parameter/alpha" in f
if e:
    del f["parameter/alpha"]
dset_Tmin = f.create_dataset("parameter/alpha", data=np.array([alpha]))
